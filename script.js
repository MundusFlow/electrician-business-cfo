...
const UPGRADE_HTML =
  ...
  '<a href="#cta">Get The Electrician Business CFO &rarr;</a>';
...
(() => {
  ...
  wireClearButton("q_clearBtn", [materialsInput, markupInput, hoursInput, rateInput, permitsInput, marginInput], btn, resultEl);
  ...
  function calculate() {
    ...
    const interpretation = `Charging ${formatMoney(quoteTotal)} covers your ${formatMoney(subtotal)} in materials, labor, and permits, and still bags you ${formatMoney(profitBuffer)} in profit &mdash; right at your ${formatPercent(marginPct)} target margin.`;
    const benchmark = "Target margins for electrical contracting jobs commonly run 25-35% (general estimate).";
...
(() => {
  ...
  wireClearButton(
    "hr_clearBtn",
    [takeHomeInput, vehicleInput, insuranceInput, toolsInput, licensingInput, otherOverheadInput, hoursPerWeekInput, weeksPerYearInput],
    btn,
    resultEl
  );
  ...
  function calculate() {
    ...
    const benchmark = "Independent electrician rates commonly range $75-$150/hr depending on region and license type (rough estimate).";
